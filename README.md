...
You must have an database configuration file under config directory instead of having database configurations in `.env` file.
create a new file in `config/database.php` and simply add the following configurations.
...
Also you must have a file under config directory named `config/iseed.php` then add the following lines in it.
...
3) Add the class alias in `app/bootstrap/app.php` file.
```
class_alias(\Whitehatsllc\Iseed\Facades\Iseed::class, 'Iseed');
```
4) Then add a new line in REGISTER SERVICE PROVIDERS
```
$app->register(Whitehatsllc\Iseed\IseedServiceProvider::class);
```
5) Simply run `composer dump-autoload` to regenerate the auto generated files.
...
Please note that some users encountered a problem with large DB table exports ([error when seeding from table with many records](https://github.com/whitehatsllc/iseed/issues/4)). The issue was solved by splitting input data into smaller chunks of elements per insert statement. As you may need to change the chunk size value in some extreme cases where DB table has a large number of columns, the chunk size is configurable in iSeed's `config.php` file:
